 ...
 * field is internal and never shown; a fallback reply renders like any
 * other reply.
 */
...
const GREETING: ChatMessage = {
  role: "assistant",
  content:
    "hi. ask me anything about aether, or about music. new releases, old records, how a page works, all fine.",
};
  ...
  return (
    <>
      ...
      <button
        type="button"
        onClick={() => setOpen((v) => !v)}
        aria-label={open ? "Close the Aether assistant" : "Open the Aether assistant"}
        aria-expanded={open}
        className={`glass fixed bottom-6 right-6 z-[85] flex h-14 w-14 items-center justify-center rounded-full transition-all duration-300 hover:border-paper/30 ${
      ...
      {open && (
        <div
          ...
          className="glass fixed bottom-24 right-6 z-[85] flex max-h-[min(34rem,70vh)] w-[min(24rem,calc(100vw-3rem))] flex-col rounded-sm"
        >
          <div className="flex items-center justify-between border-b hairline px-5 py-4">
            <span className="mono-meta text-paper/70">(AETHER)</span>
            <button
              type="button"
          ...
          <div className="flex items-center gap-2 border-t hairline p-3">
            <input
              ...
              placeholder="ask about aether, or about music…"
              aria-label="Message the Aether assistant"
